...
exports.buildTreeFromArray = buildTreeFromArray;
...
exports.decomposeProps = decomposeProps;
...
function decomposeProps(props) {
  ...
  if (config.asArray) {
    return (0, _lodash.values)(result);
  }
...
function chunkEvery(arr_, size, step) {
  var arr = (0, _cloneDeep.default)(arr_);
  var holder = [];
...
/* : Object */
{
  ...
  var acc_ = initAcc || _objectSpread(_objectSpread({}, head), {}, {
    children: []
  }); // if no acc, take first el as acc and remove it from array


  if (tail && tail.length && tail.length > 0) {
    var newAcc = _objectSpread(_objectSpread({}, acc_), {}, {
      children: [buildTreeFromArray(tail)]
    });
