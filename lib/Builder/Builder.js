...
class Builder {
  ...
  buildNodes(arr) {
    ...
    var data = arr.map(function _attemptBuildingANode(candidate) {
      ...
      newNode.properties._label = node.labels.length !== 0 ? node.labels.join("|") : null;
  ...
  buildPartialNodes(arr) {
    ...
    var data = arr.map(obj => {
      ...
      properties = (0, _.isNodeObj)(obj) ? (0, _keys.default)(properties).reduce((acc, key) => {
  ...
  makeEnhancedNode(coreNode, requiredRels, optionalRels) {
    var candidate = this.makeEnhancedNodeCandidate(coreNode, requiredRels, optionalRels);
    var result = this.buildEnhancedNodes([candidate]); // log(result)
  ...
  makeRelationshipCandidate(labels, partnerNode, properties, direction) {
    ...
    if ((0, _utils.isMissing)(direction) || (0, _utils.not)(outbound || inbound) // isMissing(direction) ||
    // not(direction == "outbound" || direction == "inbound")
    ) {
      /* default to outbound */
      direction = "outbound";
      outbound = true; // log("wassap");
      // log(partnerNode);
      // throw new Error(
