...
function getParticipatingNodes() {
  ...
  var final = current.reduce((acc, node) => {
    if (isEnhancedNode(node)) {
      acc.push(...node.getParticipatingNodes());
      return acc;
    } else if ((0, _Node.isNode)(node)) {
      /**
       ...
       */
      var matched = acc.filter(_ref => {
        var {
          properties: {
            _hash
          }
        } = _ref;
        return _hash === node.properties._hash;
      });
      ...
      if (matched.length) {
         ...
         */
        var existingKeys = (0, _keys.default)(matched[0].properties);
        var newKeys = (0, _keys.default)(node.properties);
