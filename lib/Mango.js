...
class Mango {
  ...
  buildAndMergeEnhancedNode(_ref) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      relationships = relationships || []; // log('buildAndMergeEnhancedNode relationships', relationships)

      var enode = _this7.builder.makeEnhancedNode({
        labels,
        properties
      }, relationships); // log(enode)
...
function buildDeepSimplifiedEnhancedNode(arr, customizeRelationship) {
  var [startNode, ...descendants] = arr;

  var acc = _objectSpread(_objectSpread({}, startNode), {}, {
    relationships: startNode.relationships || []
  });
  ...
  if (descendants && descendants.length) {
    ...
    if (customizeRelationship && typeof customizeRelationship === 'function') {
      return _objectSpread(_objectSpread({}, acc), {}, {
        relationships: [_objectSpread(_objectSpread({}, customizeRelationship(startNode, descendants[0])), {}, {
        ...
        ].concat(acc.relationships && (0, _lodash.isArray)(acc.relationships) ? acc.relationships : [])
      });
    }
