...
class Engine {
  constructor() {
    ...
    this.ip = config.ip || "0.0.0.0"; // this.ip = isPresent(config.ip) ? config.ip : "0.0.0.0";

    this.port = config.port || "7687"; // this.port = isPresent(config.port) ? config.port : "7687";

    this.driver = config.driver || null; // start driver explicitly

    this.database = config.database || "neo4j"; // this.database = isPresent(config.database) ? config.database : "neo4j";
        ...
        var _ref7 = (0, _asyncToGenerator2.default)(function* (arr) {
          ...
          var array_to_query = arr.map(function produceQueryObjects(node) {
            ...
            if ((0, _has.default)(node.properties, ["_hash"])) {
              var _query4 = "OPTIONAL MATCH (x".concat(node.toString("labels"), "{_hash:'").concat(node.properties._hash, "'}) RETURN x");
                ...
                if (["IN", "in"].includes(cond)) {
                  ...
                  function worker(val) {
                    ...
                    if ((0, _isArray.default)(val)) {
                      val.forEach((el, idx) => {
                        if (val.length <= 1 || // is it the last element?
                        idx == val.length - 1 // is it the last element?
                        ) {
                          holder.push("x.".concat(key, " CONTAINS ").concat(stringifyPerType(el)));
                        } else {
                          holder.push("x.".concat(key, " CONTAINS ").concat(stringifyPerType(el), " OR"));
                        }
                      });
                    } else {
  ...
  enhanceNodes(arr) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      function _validateArguments(nodes) {
        ...
        var isnode = nodes.map(_Node.isNode);
