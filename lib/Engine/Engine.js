          ...
          var data = yield Promise.all(array_to_query.map( /*#__PURE__*/function () {
            var _ref8 = (0, _asyncToGenerator2.default)(function* (queryObject) {
              ...
              .map(node => {
                ...
                log('if Neo4j returned no matches, its represented as []');
              ...
              .map(condition => {
                ...
                if (["IN", "in"].includes(cond)) {
                  ...
                  function worker(val) {
                    ...
                    holder.push("x.".concat(key, " CONTAINS ").concat(stringifyPerType((0, _isString.default)(val) ? val : (0, _isArray.default)(val) && val.length === 1 ? val[0] : (0, _isArray.default)(val) ? Array.from(val).forEach((el, idx) => {
  ...
  matchRelationships(arr) {
    ...
    return (0, _asyncToGenerator2.default)(function* () {
      ...
      var result = dbData.map((result, i) => {
        ...
        if (!data.length) {
          log('if there were no Relationships, it is a Failure');
          return new _Result.Failure({
            reason: NEO4J_RETURNED_NULL,
            parameters: {
              rel: arr[i]
