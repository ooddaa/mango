...
*   `config` **[Object][33]** Configuration object.
    ...
    *   `config.engineConfig` **[Object][33]** Configuration object to instantiate connection to Neo4j.

        *   `config.engineConfig.username` **[string][34]** Neo4j DBMS username.
        *   `config.engineConfig.password` **[string][34]** Neo4j DBMS password.
        *   `config.engineConfig.ip` **[string][34]** Neo4j DBMS IP to connect to. Default: '0.0.0.0'.
        *   `config.engineConfig.port` **[string][34]** Neo4j DBMS port to connect to. Default: '7687'.
        *   `config.engineConfig.database` **[string][34]** Neo4j DBMS database name to use. Default: 'neo4j'.
    *   `config.builder` **Builder** Instance of a Builder class. If none is supplied, a generic one will be instantiated for you.
...
*   `labels` **[Array][35]<[string][34]>** Array of labels to match Nodes by.
*   `props` **[Object][33]** A POJO containing key:value to search for matches.
*   `config` **[Object][33]** Configuration object. (optional, default `{}`)

    *   `config.exactMatch` **[boolean][36]** {true} Mango searches for exactly what we specified, ie the returned Nodes must not have any extra properties above supplied labels & properties. You get exactly what you asked for.{false} Mango matches any Nodes that have supplied labels and properties and the returned result may contain Nodes with extra properties. You may get more than you asked for. (optional, default `false`)
    *   `config.returnResult` **[boolean][36]** {true} returns a Result with additional Neo4j query data.{false} return Node\[]. (optional, default `false`)
...
Returns **[Promise][37]<(Result | [Array][35]<[Node][38]>)>** 
...
*   `labels` **[Array][35]<[string][34]>** Array of labels to match Nodes by.
*   `props` **[Object][33]** A POJO with properties to construct and merge a Node to Neo4j. !MUST contain at least one label and a REQUIRED property to be unique (Uniqueness Requirement).
*   `config` **[Object][33]** Configuration object. (optional, default `{}`)

    *   `config.returnResult` **[boolean][36]** {true} returns a Result with additional Neo4j query data.{false} returns Node\[]. (optional, default `false`)
...
Returns **[Promise][37]<(Result | EnhancedNode)>** 
...
*   `startNode` **([Node][38] | EnhancedNode)** Node that has an outbound Relationship.
...
*   `endNode` **([Node][38] | EnhancedNode)** Node that has an inbound Relationship.
*   `config` **[Object][33]** Configuration object. (optional, default `{}`)

    *   `config.returnResult` **[boolean][36]** {true} returns a Result with additional Neo4j query data.{false} returns Relationship. (optional, default `false`)
...
Returns **[Promise][37]<(Result | Relationship)>** 
...
*   `graphPattern` **SimplifiedEnhancedNodeObject** a graph pattern (aka EnhancedNode) to be built and merged into Neo4j.
*   `config` **[Object][33]**  (optional, default `{}`)
...
*   `node` **([Node][38] | EnhancedNode)** Node to delete.
*   `config` **[Object][33]** Configuration object. (optional, default `{}`)

    *   `config.archiveNodes` **[boolean][36]** {true} does not delete anything. Instead it sets { \_isCurrent: false, \_dateArchived: TimeArray } on Node & all its Relationships.{false} permanently deletes Nodes and all its Relations. (optional, default `false`)
...
Returns **[Promise][37]\<Result>** Returns Result where Result.data contains affected Node and its Relationships.
...
*   `condition` **[string][34]** Condition for value search ( > < >= <= == etc. ).
*   `value` **any** Searched value.

Returns **[ConditionContainer][39]** 
...
Returns **{requiredProps: [Object][33], optionalProps: [Object][33], privateProps: [Object][33]}** 
...
Returns **[Object][33]** { condition: value }.
